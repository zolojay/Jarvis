...
def with_db_connection(func):
    ...
    Parameters:
    func: Function that requires a dat+abase connection
...
@with_db_connection
def get_daily_completions(conn, weeks_ago=0):
    """
    Get completions for each day of the specified week
    ...
    Returns:
    DataFrame: DataFrame with completions for each day of the week
    """
    if conn is None:
        return pd.DataFrame()
    ...
    # Query to get completed tests by day
    query = """
    SELECT 
        date(LS.load_end) as completion_date,
        SUM((SELECT COUNT(DISTINCT RTST_inner.id) 
             FROM ReactorTests RTST_inner 
             LEFT JOIN ReactorTestConditions RTC_inner ON RTST_inner.test_condition_id = RTC_inner.id
             LEFT JOIN TestConditionTypes TCT_inner ON RTC_inner.test_type_id = TCT_inner.id
             WHERE RTST_inner.load_id = RL.id 
             AND (TCT_inner.test_condition_type IS NULL OR TRIM(UPPER(TCT_inner.test_condition_type)) <> 'PRESSURE DROP'))) as test_count,
        COUNT(DISTINCT RL.id) as load_count
    FROM ReactorLoads RL
    INNER JOIN LoadBench LB ON RL.id = LB.load_id
    INNER JOIN LoadSchedules LS ON RL.id = LS.load_id
    WHERE LS.load_end BETWEEN ? AND ?
    AND (LB.status = 'Test Complete' OR LB.status = 'QC Complete' OR LB.status = 'Report Delivered')
    GROUP BY date(LS.load_end)
    ORDER BY date(LS.load_end)
    """
    ...
    try:
        ...
        if df.empty:
            ...
            df = pd.DataFrame(columns=['completion_date', 'test_count', 'load_count'])
        else:
            # Convert completion_date to datetime for proper handling
        ...
        # Merge with our results to fill in missing days
        df = pd.merge(date_df, df, on='completion_date', how='left')
        
        # Fill NaN values with 0
        df['test_count'] = df['test_count'].fillna(0).astype(int)
        df['load_count'] = df['load_count'].fillna(0).astype(int)
        
        # Add day of week names
        df['day_name'] = df['completion_date'].dt.day_name()
        
        return df
    except Exception as e:
        st.error(f"Error getting daily completions: {e}")
        return pd.DataFrame(columns=['completion_date', 'test_count', 'load_count', 'day_name'])
...
def get_weekly_completions(conn, weeks_ago=0):
    ...
    """
    if conn is None:
        return {"week_label": "", "test_count": 0, "load_count": 0}
        
    week_start, week_end = get_week_dates(weeks_ago)
    
    # Query to get completed tests
    test_query = """
    SELECT SUM((SELECT COUNT(DISTINCT RTST_inner.id) 
                FROM ReactorTests RTST_inner 
                LEFT JOIN ReactorTestConditions RTC_inner ON RTST_inner.test_condition_id = RTC_inner.id
                LEFT JOIN TestConditionTypes TCT_inner ON RTC_inner.test_type_id = TCT_inner.id
                WHERE RTST_inner.load_id = RL.id 
    ...
    try:
        ...
        st.error(f"Error getting weekly completions: {e}")
        return {
            'week_label': f"{week_start.strftime('%b %d')} - {week_end.strftime('%b %d')}",
            'test_count': 0,
            'load_count': 0
...
def get_weekly_completions_by_request_type(conn, weeks_ago=0):
    ...
    try:
        ...
        st.error(f"Error getting weekly completions by request type: {e}")
        return pd.DataFrame(columns=['request_type', 'test_count', 'load_count'])
...
def get_weekly_completions_by_condition_type(conn, weeks_ago=0):
    ...
    SELECT 
        ...
        COUNT(DISTINCT RL.id) as load_count
    FROM ReactorLoads RL
    INNER JOIN ReactorTests RTest ON RTest.load_id = RL.id
    ...
    try:
        ...
        st.error(f"Error getting weekly completions by condition type: {e}")
        return pd.DataFrame(columns=['test_condition_type', 'test_count', 'load_count'])
...
def get_monthly_completions(conn, months_ago=0):
    ...
    """
    if conn is None:
        return {"month_label": "", "test_count": 0, "load_count": 0}
    ...
    try:
        ...
        st.error(f"Error getting monthly completions: {e}")
        return {
            'month_label': month_start.strftime('%B %Y'),
            'test_count': 0,
            'load_count': 0
...
def get_monthly_completions_by_request_type(conn, months_ago=0):
    ...
    try:
        ...
        st.error(f"Error getting monthly completions by request type: {e}")
        return pd.DataFrame(columns=['request_type', 'test_count', 'load_count'])
...
def get_monthly_completions_by_condition_type(conn, months_ago=0):
    ...
    try:
        ...
        st.error(f"Error getting monthly completions by condition type: {e}")
        return pd.DataFrame(columns=['test_condition_type', 'test_count', 'load_count'])

# Get Current In-Reactor Load
@st.cache_data(ttl=300)  # Cache for 5 minutes
@with_db_connection
...
def display_week_selector(key_prefix=""):
    """
    Display a week selector with options for This Week, Last Week, etc.
    
    Parameters:
    key_prefix (str): Prefix for Streamlit widget keys to ensure uniqueness
    
    Returns:
    int: Number of weeks ago (0 = current week, 1 = last week, etc.)
    """
    # Check if state key exists, initialize if it doesn't
    if f"selected_week_{key_prefix}" not in st.session_state:
        st.session_state[f"selected_week_{key_prefix}"] = 0
    ...
    with col1:
        selected_option = st.selectbox(
            ...
            index=list(week_options.values()).index(st.session_state[f"selected_week_{key_prefix}"]),
            key=f"week_selector_{key_prefix}"
        )
    
    with col2:
        apply_week = st.button("Apply", key=f"apply_week_{key_prefix}")
    
    # Update session state if Apply is clicked
    if apply_week:
        st.session_state[f"selected_week_{key_prefix}"] = week_options[selected_option]
    
    return st.session_state[f"selected_week_{key_prefix}"]
...
def display_weekly_completions():
    """Display the weekly completions section with table and chart"""
    st.subheader("📊 Weekly Completions")
    
    # Week selector
    selected_weeks_ago = display_week_selector("weekly_completions")
    
    # Get data for selected week
    week_start, week_end = get_week_dates(selected_weeks_ago)
    
    # Get daily completions for the selected week
    daily_completions = get_daily_completions(selected_weeks_ago)
    
    # Check if we have data
    if daily_completions.empty:
        st.info(f"No load completions found for the selected week ({week_start.strftime('%b %d')} - {week_end.strftime('%b %d')}).")
        return
    
    # Summary metrics
    weekly_data = get_weekly_completions(selected_weeks_ago)
    
    # Display summary in metrics
    col1, col2 = st.columns(2)
    with col1:
        st.metric("Tests Completed", weekly_data['test_count'])
    with col2:
        st.metric("Loads Completed", weekly_data['load_count'])
    
    # Create daily breakdown chart
    st.subheader(f"Daily Breakdown ({week_start.strftime('%b %d')} - {week_end.strftime('%b %d')})")
    
    # Format the data for the chart
    daily_completions['day_label'] = daily_completions['completion_date'].dt.strftime('%a %b %d')
    ...
    with st.expander("Show Detailed Daily Data", expanded=False):
        # Create a nicer table version
        table_data = daily_completions[['day_name', 'completion_date', 'test_count', 'load_count']].copy()
        table_data['completion_date'] = table_data['completion_date'].dt.strftime('%Y-%m-%d')
        
        # Configure columns
        column_config = {
            'day_name': st.column_config.TextColumn("Day", width='medium'),
            'completion_date': st.column_config.TextColumn("Date", width='medium'),
            'test_count': st.column_config.NumberColumn("Tests Completed", width='medium'),
            'load_count': st.column_config.NumberColumn("Loads Completed", width='medium'),
        }
    ...
    request_type_data = get_weekly_completions_by_request_type(selected_weeks_ago)
    
    if not request_type_data.empty:
        st.subheader("Completions by Request Type")
...
def display_dashboard():
    ...
    tab1, tab2, tab3 = st.tabs(["Current Status", "Weekly Completions", "Monthly Overview"])
    
    # Tab 1: Current Status
    with tab1:
        # Replace the current "Current In-Reactor Load" section with QB and FB statuses
        st.subheader("Current Testing Status")
        
        # Create two columns for QB and FB
        qb_col, fb_col = st.columns(2)
        
        # Quarter Bench Status
        with qb_col:
            st.subheader("Quarter Bench")
            qb_load = get_current_qb_load()
            
            if qb_load:
                st.markdown(f"""
                <div class="metric-card" style="background-color: #E3F2FD; border-left: 4px solid #0D47A1;">
                    <h3 style="color: #0D47A1;">Load {qb_load['ReactorLoadID']} - {qb_load['LabRequestNumber']}</h3>
                    <p><strong>Status:</strong> <span style="color: #0D47A1; font-weight: bold;">In Reactor</span></p>
                    <p><strong>Started:</strong> {qb_load['start_time']}</p>
                    <p><strong>PCN:</strong> {qb_load['pcn']}</p>
                    <p><strong>Job Number:</strong> {qb_load['job_number']}</p>
                    <p><strong>Sample Types:</strong> {qb_load['sample_types']}</p>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.info("No current Quarter Bench load found.")
        
        # Full Bench Status
        with fb_col:
            st.subheader("Full Bench")
            fb_loads = get_current_fb_loads()
            
            for reactor in ['r1', 'r2', 'r3']:
                reactor_load = fb_loads.get(reactor)
                
                if reactor_load:
                    st.markdown(f"""
                    <div class="metric-card" style="background-color: #F3E5F5; border-left: 4px solid #6A1B9A; margin-bottom: 10px;">
                        <h3 style="color: #6A1B9A;">Reactor {reactor.upper()}: Load {reactor_load['ReactorLoadID']} - {reactor_load['LabRequestNumber']}</h3>
                        <p><strong>Started:</strong> {reactor_load['start_time']}</p>
                        <p><strong>PCN:</strong> {reactor_load['pcn']}</p>
                        <p><strong>Job Number:</strong> {reactor_load['job_number']}</p>
                        <p><strong>Sample Types:</strong> {reactor_load['sample_types']}</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown(f"""
                    <div class="metric-card" style="background-color: #EFEBE9; border-left: 4px solid #4E342E; margin-bottom: 10px;">
                        <h3 style="color: #4E342E;">Reactor {reactor.upper()}</h3>
                        <p>No current load</p>
                    </div>
                    """, unsafe_allow_html=True)
    
    # Tab 2: Weekly Completions Section
    with tab2:
        display_weekly_completions()
    ...
    with tab3:
        ...
        prev_month = get_monthly_completions(1)
        
        # Create metrics cards for current month with comparison to previous month
        col1, col2 = st.columns(2)
        
        with col1:
            # Calculate change from previous month for tests
            prev_test_count = prev_month['test_count']
            curr_test_count = current_month['test_count']
            test_delta = None
            if prev_test_count > 0:
                test_delta = f"{((curr_test_count - prev_test_count) / prev_test_count) * 100:.1f}% from previous month"
        ...
        with col2:
            # Calculate change from previous month for loads
            prev_load_count = prev_month['load_count']
            curr_load_count = current_month['load_count']
            load_delta = None
            if prev_load_count > 0:
                load_delta = f"{((curr_load_count - prev_load_count) / prev_load_count) * 100:.1f}% from previous month"
...
# Get current Quarter Bench load
@st.cache_data(ttl=300)  # Cache for 5 minutes
@with_db_connection
def get_current_qb_load(conn):
    """
    Get the current Quarter Bench load that has a valid start time
    
    Parameters:
    conn (sqlite3.Connection): Database connection
    
    Returns:
    dict: Dictionary containing load information or None if no load is found
    """
    if conn is None:
        return None
    ...
    LEFT JOIN LabRequestSample LRS ON RLS.lab_request_sample_id = LRS.id
    LEFT JOIN Samples S ON LRS.sample_id = S.id
    WHERE LB.testing_area = 'Quarter Bench'
    AND LS.load_start IS NOT NULL 
    AND (LS.load_end IS NULL OR LS.load_end = '')
    GROUP BY RL.id
    ORDER BY LS.load_start DESC
    LIMIT 1
    ...
    try:
        ...
        st.error(f"Error getting current QB load: {e}")
        return None

# Get current Full Bench loads by reactor
@st.cache_data(ttl=300)  # Cache for 5 minutes
@with_db_connection
def get_current_fb_loads(conn):
    """
    Get the current Full Bench loads for each reactor (r1, r2, r3)
    
    Parameters:
    conn (sqlite3.Connection): Database connection
    
    Returns:
    dict: Dictionary mapping each reactor to its load information
    """
    if conn is None:
        return {}
    ...
    LEFT JOIN LabRequestSample LRS ON RLS.lab_request_sample_id = LRS.id
    LEFT JOIN Samples S ON LRS.sample_id = S.id
    WHERE LB.testing_area = 'Full Bench'
    AND LS.load_start IS NOT NULL 
    AND LS.reactor IN ('r1', 'r2', 'r3')
    AND (LS.load_end IS NULL OR LS.load_end = '')
    GROUP BY RL.id
    ORDER BY LS.reactor, LS.load_start DESC
    """
    ...
    try:
        ...
        if df.empty:
            return {reactor: None for reactor in ['r1', 'r2', 'r3']}
        ...
        # For each reactor, get the most recent load
        fb_loads = {}
        
        for reactor in ['r1', 'r2', 'r3']:
            reactor_df = df[df['reactor'] == reactor]
            if not reactor_df.empty:
                fb_loads[reactor] = reactor_df.iloc[0].to_dict()
            else:
                fb_loads[reactor] = None
        
        return fb_loads
    except Exception as e:
        st.error(f"Error getting current FB loads: {e}")
        return {reactor: None for reactor in ['r1', 'r2', 'r3']}
